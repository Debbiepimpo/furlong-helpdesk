...
def view_ProfessionalServices(request):
    """View to display all ProfessionalServices excluding those with cancelled or done status"""
...
def view_completed_ProfessionalServices(request):
    ...
    ProfessionalServices = PServices.objects.all().order_by('-id').filter(status='Done')
    paginator = Paginator(ProfessionalServices, 5)  # Show 5 ProfessionalServices per page
...
def ProfService_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        comments = PServicesComment.objects.filter(ProfService__pk=ProfService.pk)
...
def delete_ProfService_comment(request, pk):
    comment = get_object_or_404(PServicesComment, pk=pk)
